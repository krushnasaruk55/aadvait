...
const AppState = {
    ...
    get communities() {
        const stored = localStorage.getItem('studyhub_communities');
        return stored ? JSON.parse(stored) : null;
    },
    set communities(value) {
        localStorage.setItem('studyhub_communities', JSON.stringify(value));
    }
};
...
// ==================== Authentication ====================
function checkAuth() {
    const currentUser = localStorage.getItem('studyhub_current_user');
...
function getCurrentUser() {
    const userStr = localStorage.getItem('studyhub_current_user');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Update profile link in nav if user is logged in
    const user = getCurrentUser();
    if (user) {
        // You might want to update UI elements here with user info
    }
